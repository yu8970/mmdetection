...
from mmdet import deform_pool_cuda
    ...
    def forward(self, data, rois):
        ...
        if n == 0:
            return data.new_empty(
                n, self.out_channels, self.out_size[0], self.out_size[1]
            )
        if self.no_trans:
            offset = data.new_empty(0)
            return deform_roi_pooling(
...
class DeltaRPooling(DeformRoIPooling):
    def __init__(
        ...
        super(DeltaRPooling, self).__init__(
            spatial_scale,
            out_size,
            out_channels,
            no_trans,
    ...
    def forward(self, data, rois, delta_r):
        ...
        if self.no_trans:
            ...
            n = rois.shape[0]
            offset_tmp = delta_r.view(n, 2, 1, 1).repeat(
                1, 1, self.out_size, self.out_size
            )
...
class DeltaCPooling(DeformRoIPooling):
    def __init__(
        ...
        super(DeltaCPooling, self).__init__(
            spatial_scale,
            out_size,
            out_channels,
            no_trans,
