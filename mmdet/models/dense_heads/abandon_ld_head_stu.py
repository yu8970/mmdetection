...
class LDHead(GFLHead):
    ...
    def loss_by_feat_single(self, anchors: Tensor, cls_score: Tensor,
        ...
        if len(pos_inds) > 0:
            ...
            # 解耦模块 reg
            loss_reg_aban = self.loss_reg_aban(
                pred_corners,
                soft_corners,
                weight=weight_targets[:, None].expand(-1, 4).reshape(-1),
                avg_factor=4.0)
            ...
            loss_cls_aban = bbox_pred.sum() * 0
            loss_reg_aban = bbox_pred.sum() * 0
        ...
        return loss_cls, loss_bbox, loss_dfl, loss_ld, loss_cls_aban, loss_reg_aban, weight_targets.sum()
    ...
    def loss(self, x: List[Tensor], out_teacher: Tuple[Tensor],
        ...
        cls_aban = out_teacher[2]
        reg_aban = out_teacher[3]
        loss_inputs = outs + (batch_gt_instances, batch_img_metas,
                              soft_targets, cls_aban, reg_aban)
    ...
    def loss_by_feat(
            ...
            cls_aban: List[Tensor],
            reg_aban: List[Tensor],
        ...
        losses_cls, losses_bbox, losses_dfl, losses_ld, losses_cls_aban, losses_reg_aban, \
